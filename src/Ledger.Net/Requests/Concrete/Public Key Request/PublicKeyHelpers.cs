...
namespace Ledger.Net.Requests.Helpers
{
    ...
    {
        public static byte[] GetDerivationPathData(
            uint coinNumber,
            uint account,
            uint index,
            bool isChange,
        ...
        private static uint[] GetDerivationIndices(
            uint coinNumber,
            uint account,
            uint index,
            bool isChange,
            bool isSegwit)
        {
            ...
            indices[3] = coinNumber != 60 ? isChange ? 1 : (uint)0 : index; // If coinNumber == 60 (ethereum) we use index instead of change.

            if (indices.Length == 5) // If coinNumber is != 60, we don't have Ethereum, so add in the address index to the last index of array
                indices[4] = index;
